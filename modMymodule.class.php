...
class modMymodule extends DolibarrModules
{
    ...
    public function init($options = '')
    {
        ...
        $myTmpObjects = array(
            ...
            'Expedition' => array('includerefgeneration' => 0, 'includedocgeneration' => 1) // << ADDED for Shipping templates
        );
        ...
        foreach ($myTmpObjects as $myTmpObjectKey => $myTmpObjectArray) {
            if ($myTmpObjectArray['includedocgeneration']) {
                ...
                // This part for ODT templates is optional if you only use PHP models
                $template_dir = ($type == 'facture') ? 'invoices' : (($type == 'commande') ? 'orders' : (($type == 'mo') ? 'mrp' : 'shipping'));
                ...
                $models = ($type == 'facture') ?
                    ...
                    (($type == 'expedition') ?
                    array('pdf_custom_shipping_slip' => 'shipping') : // << ADDED Expedition template
                    array())));
    ...
    {
        global $conf; // << ADDED to ensure $conf is available
        ...
        $sql = array(
            ...
            // Remove Expedition template
            "DELETE FROM ".MAIN_DB_PREFIX."document_model WHERE nom = 'pdf_custom_shipping_slip' AND type = 'shipping' AND entity = ".((int) $conf->entity) // << ADDED
        );
